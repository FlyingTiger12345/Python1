...
MAIN,POP = "Gesture-Controlled Photo App","Capture(ESC/Close to resume)"


pause = False;freeze= None
...
if not cap.isOpened(): print("Error;Couldd not access"); exit()
...
while True:
    if pause:
        ...
        if k == ord("q"): break
        if k ==27:
            pause = False; pinch_on = False
            try: cv2.destroyWindow(POP)
            except :pass
            continue
        try:
            if cv2.getWindowProperty(POP,cv2.WND_PROP_VISIBLE) <=0: pause = False; pinch_on = False
        except cv2.error:
            pause = False; pinch_on = False
        continue
...
#Converts normalized coordinates → pixel coordinates.

        lm = hand.landmark; tips = {k:(int(lm[v].x*w), int(lm[v].y*h)) for k,v in ids.items()}

#If thumb and index finger touch, capture image.

        tx,ty = tips["thumb"]; ix,iy = tips["index"]

#Checks if thumb touches another finger.
